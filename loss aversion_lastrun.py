...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\umesh\\OneDrive\\Desktop\\untitled_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instructions = visual.TextStim(win=win, name='instructions',
        text="Read the scenario carefully and press any key for options.\nPress 'a' for chosing 'option A' and press 'b' for chosing 'option B'.\nClick on the displayed square to continue\n  press any key to proceed:)",
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    # --- Initialize components for Routine "displaying_scenario" ---
    ...
    options = visual.TextStim(win=win, name='options',
        text='Option A:Accept it and walk away.\nOption B:Continue playing so that you may gain money or lose more money.\n  Press the key corresponding to the option you want to chose.\n',
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('loss aversion.xlsx'),
    ...
    for thisTrial in trials:
        ...
        # --- Prepare to start Routine "displaying_scenario" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('displaying_scenario.started', globalClock.getTime(format='float'))
        scenario.setText(Scenario)
        key_resp.keys = []
        key_resp.rt = []
        _key_resp_allKeys = []
        # keep track of which components have finished
        displaying_scenarioComponents = [scenario, options, key_resp]
        for thisComponent in displaying_scenarioComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "displaying_scenario" ---
        routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            ...
            # if scenario is stopping this frame...
            if scenario.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > scenario.tStartRefresh + 5-frameTolerance:
                    # keep track of stop time/frame for later
                    scenario.tStop = t  # not accounting for scr refresh
                    scenario.tStopRefresh = tThisFlipGlobal  # on global time
                    scenario.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'scenario.stopped')
                    # update status
                    scenario.status = FINISHED
                    scenario.setAutoDraw(False)
            
            # *options* updates
            
            # if options is starting this frame...
            if options.status == NOT_STARTED and tThisFlip >= 5-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if options is stopping this frame...
            if options.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > options.tStartRefresh + 10-frameTolerance:
                    # keep track of stop time/frame for later
                    options.tStop = t  # not accounting for scr refresh
                    options.tStopRefresh = tThisFlipGlobal  # on global time
                    options.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'options.stopped')
                    # update status
            ...
            if key_resp.status == NOT_STARTED and tThisFlip >= 5.5-frameTolerance:
                # keep track of start time/frame for later
        ...
        # --- Ending Routine "displaying_scenario" ---
        for thisComponent in displaying_scenarioComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('displaying_scenario.stopped', globalClock.getTime(format='float'))
        # check responses
        ...
        # the Routine "displaying_scenario" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        
        # --- Prepare to start Routine "square" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('square.started', globalClock.getTime(format='float'))
        ...
        # keep track of which components have finished
        squareComponents = [polygon, mouse]
        ...
        while continueRoutine and routineTimer.getTime() < 5.0:
            ...
            if mouse.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > mouse.tStartRefresh + 4.5-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        for thisComponent in squareComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('square.stopped', globalClock.getTime(format='float'))
